...
void CCS_BLOCK(aes_cxx_catapult)(
    ...
    uint32_t dma_read_data_length_key = 0;
    uint32_t dma_read_data_length_iv = 0;
    uint32_t dma_read_data_length_in = 0;
    ...
    for (unsigned b = 0; b < batch; b++) {
        ...
        dma_read_data_length_key = (key_bytes + 4 - 1) / 4; // ceil(key_bytes / 4)

        // ESP_REPORT_INFO(VON, "DMA read data length = %u",  ESP_TO_UINT32(dma_read_data_length));

        if (oper_mode == CTR_OPERATION_MODE || oper_mode == CBC_OPERATION_MODE) {
            dma_read_data_length_iv = (iv_bytes + 4 - 1) / 4; // ceil(iv_bytes / 4)
        }

        dma_read_data_length_in = (in_bytes + 4 - 1) / 4; // ceil(key_bytes / 4)

        dma_read_data_length = dma_read_data_length_key + dma_read_data_length_iv + dma_read_data_length_in;

        ESP_REPORT_INFO(VON, "Final DMA read data length = %u",  ESP_TO_UINT32(dma_read_data_length));
...
LOAD_KEY_LOOP:
            for (i = 0; i < PLM_KEY_SIZE; i+=2) {

                if (i >= dma_read_data_length_key) break;

                assert(DMA_WIDTH == 64 && "DMA_WIDTH should be 64 bits");

                ac_int<DMA_WIDTH, false> data_dma;

                // ESP_REPORT_INFO(VON, "Before check on channel availability");

#ifndef __SYNTHESIS__
                while (!dma_read_chnl.available(1)) {}; // Hardware stalls until data ready
#endif

                // ESP_REPORT_INFO(VON, "After check on channel availability");
                ...
                ESP_REPORT_INFO(VON, "plm_key[%u] = %02X", ESP_TO_UINT32(i)+0, data_0.to_uint());
                ESP_REPORT_INFO(VON, "plm_key[%u] = %02X", ESP_TO_UINT32(i)+1, data_1.to_uint());
            }

            if (oper_mode == CTR_OPERATION_MODE || oper_mode == CBC_OPERATION_MODE) {
LOAD_IV_LOOP:
                for (j = 0; j < PLM_IN_SIZE; j+=2) {

                    if (j >= dma_read_data_length_iv) break;
                    ...
                    data_0 = data_dma.template slc<WL>(WL*0).to_uint();
                    data_1 = data_dma.template slc<WL>(WL*1).to_uint();
                    plm_iv.data[j+0] = data_0;
                    plm_iv.data[j+1] = data_1;

                    ESP_REPORT_INFO(VON, "plm_iv[%u] = %02X", ESP_TO_UINT32(j)+0, data_0.to_uint());
                    ESP_REPORT_INFO(VON, "plm_iv[%u] = %02X", ESP_TO_UINT32(j)+1, data_1.to_uint());
                }
            }

LOAD_INPUT_LOOP:
            for (k = 0; k < PLM_IN_SIZE; k+=2) {

                // ESP_REPORT_INFO(VON, "Inside plm_in loop before break");

                if (k >= dma_read_data_length_in) break;

                assert(DMA_WIDTH == 64 && "DMA_WIDTH should be 64 bits");

                ac_int<DMA_WIDTH, false> data_dma;

                // ESP_REPORT_INFO(VON, "Inside plm_in loop");
                ...
                data_0 = data_dma.template slc<WL>(WL*0).to_uint();
                data_1 = data_dma.template slc<WL>(WL*1).to_uint();
                plm_in.data[k+0] = data_0;
                plm_in.data[k+1] = data_1;

                ESP_REPORT_INFO(VON, "plm_in[%u] = %02X", ESP_TO_UINT32(k)+0, data_0.to_uint());
                ESP_REPORT_INFO(VON, "plm_in[%u] = %02X", ESP_TO_UINT32(k)+1, data_1.to_uint());
            }
