...
int run_brain()
{
	...
	/* key_num = MAX_ITER; */
        ...
        ((struct brain_bit_alt_vivado_access*) cfg_000[0].esp_desc)->key_num = MAX_ITER;
        ((struct brain_bit_alt_vivado_access*) cfg_000[0].esp_desc)->key_length = KEY_LEN;
	...
	printf("Randomness source: brain_bit_alt\n");
	printf("Number of random bits: %d\n", KEY_LEN);
	printf("Time measured: %u Iterations, avg time %.4f us.\n", MAX_ITER, avg_elapsed_us);
...
int main(int argc, char *argv[])
{
    if(argc < 2 || *argv[1] == '1') {
        ...
        printf("running brain_bit_alt \n");
        run_brain();
    }
    else if (*argv[1] == '5') {
        printf("running brain_bit_alt software\n");
        run_brain_sw();
    }
