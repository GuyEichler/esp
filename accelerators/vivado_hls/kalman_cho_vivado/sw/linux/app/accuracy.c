...
int main(int argc, char **argv)
{
	...
	x_dim = 6;
	((struct kalman_cho_vivado_access*) cfg_000[0].esp_desc)->x_dim = x_dim;
	z_dim = 164;
	((struct kalman_cho_vivado_access*) cfg_000[0].esp_desc)->z_dim = z_dim;
	iter = 100;
	((struct kalman_cho_vivado_access*) cfg_000[0].esp_desc)->iter = iter;
	chunks = 1;
	((struct kalman_cho_vivado_access*) cfg_000[0].esp_desc)->chunks = chunks;
	...
	for(int inv_num = 1; inv_num < 7; inv_num++)
		...
		{
			((struct kalman_cho_vivado_access*) cfg_000[0].esp_desc)->inv_num = inv_num;
			((struct kalman_cho_vivado_access*) cfg_000[0].esp_desc)->inv_reset = inv_reset;
			printf("running x_dim=%d z_dim=%d iter=%d chunks=%d inv_num=%d inv_reset=%d \n", x_dim, z_dim, iter, chunks, inv_num, inv_reset);
			...
			printf("\n====== %s ======\n\n", cfg_000[0].devname);
			/* <<--print-params-->> */
			printf("  .inv_reset = %d\n", ((struct kalman_cho_vivado_access*) cfg_000[0].esp_desc)->inv_reset);
			printf("  .inv_num = %d\n", ((struct kalman_cho_vivado_access*) cfg_000[0].esp_desc)->inv_num);
			printf("  .chunks = %d\n", ((struct kalman_cho_vivado_access*) cfg_000[0].esp_desc)->chunks);
			printf("  .iter = %d\n", ((struct kalman_cho_vivado_access*) cfg_000[0].esp_desc)->iter);
			printf("  .x_dim = %d\n", ((struct kalman_cho_vivado_access*) cfg_000[0].esp_desc)->x_dim);
			printf("  .z_dim = %d\n", ((struct kalman_cho_vivado_access*) cfg_000[0].esp_desc)->z_dim);
			printf("\n  ** START **\n");
