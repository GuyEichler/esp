...
load_data:
    ...
    load_initz0:for(unsigned k = 0; k < CHUNK_MAX; k++)
    load_initz1:for(unsigned j = 0; j < Z_MAX; j++)
        {
            Z[k][j] = 0.0;
        }
...
compute_data:
    ...
    static comp_word_t S_inv_final[Z_MAX][Z_MAX];
    static comp_word_t S_inv_final2[Z_MAX][Z_MAX];
    int inv_ok = 0;
    ...
    LOOP_S_inv_2:for(int j = 0; j < Z_MAX; j++)
        {
            if(i == j && i < z_dim)
            {
                ...
                {//Comment this block to change policy
                    ...
                    comp_word_t tmp2 = S_inv_final2[i][j];
                    if(inv_num % 2 == 0)// && c > 1)
                        S_inv[i][j] = tmp2;
                    else// if (c > 1)
                        S_inv[i][j] = tmp;
    ...
    hls::matrix_multiply_top<hls::NoTranspose, hls::NoTranspose,
        ...
        if(inv_num % 2 == 0){
            hls::matrix_multiply_top<hls::NoTranspose, hls::NoTranspose,
                             ...
                             comp_word_t, comp_word_t> (inter4, S_inv_final2, K);
        }
        else{
            hls::matrix_multiply_top<hls::NoTranspose, hls::NoTranspose,
                             X_MAX, Z_MAX, Z_MAX, Z_MAX,
                             ...
                             TRAITS_E,
                             // word_t, word_t> (inter4, S_inv, K);
                             comp_word_t, comp_word_t> (inter4, S_inv_final, K);
        }
    }
...
// #endif
        ...
        else if(i < out_length)
        {
            ...
            comp_word_t tmp;// = P_pred[row][col];
            //_outbuff[i] = tmp.to_float();
            if(pingpong)
                tmp = P_pred_pong[row][col];
            else
    ...
    // printf("X_pred = \n");
    // if(pingpong)
    //     hls::print_matrix<1, X_MAX, comp_word_t, hls::NoTranspose>((comp_word_t(*)[X_MAX])X_pred_pong, "   ");
    // else
    //     hls::print_matrix<1, X_MAX, comp_word_t, hls::NoTranspose>((comp_word_t(*)[X_MAX])X_pred_ping, "   ");
#endif

#ifndef __SYNTHESIS__
    // printf("P_pred = \n");
    // if(pingpong)
    //     hls::print_matrix<X_MAX, X_MAX, comp_word_t, hls::NoTranspose>((comp_word_t(*)[X_MAX])P_pred_pong, "   ");
    // else
    //     hls::print_matrix<X_MAX, X_MAX, comp_word_t, hls::NoTranspose>((comp_word_t(*)[X_MAX])P_pred_ping, "   ");
#endif
