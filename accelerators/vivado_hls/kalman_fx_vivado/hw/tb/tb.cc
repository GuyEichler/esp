...
int main(int argc, char **argv) {
    ...
    for(unsigned i = 0; i < iter; i++)
        for(unsigned j = 0; j < x_dim + x_dim * x_dim; j++)
        {
            ...
            // else
            //     std::cout << "OKAY: P Accelerator value: " << acc_val << " Golden value: " << gold_val << " index: " << i * out_words_adj + j << " Iter " << i << " diff: " << diff << std::endl;
	    ...
	    if (outbuff[i * out_words_adj + j] != outbuff_gold[i * out_words_adj + j]){
                if(diff/gold_val > 0.1 || diff/acc_val > 0.1 || diff/gold_val < -0.1 || diff/acc_val < -0.1){
                    ...
                    if(j < x_dim)
                        ...
                        std::cout << "BAD: P Accelerator value: " << acc_val << " Golden value: " << gold_val << " index: " << i * out_words_adj + j << " Iter " << i << " diff: " << diff << std::endl;
