...
static int validate_buffer(token_t *out, float *gold)
{
	...
	for (i = 0; i < iter; i++)
		for (j = 0; j < (x_dim + x_dim * x_dim)*chunks; j++)
		{
			...
			float acc_val = fixed32_to_float(out[i * out_words_adj + j], 16);
...
static float accuracy_validate_buffer(token_t *out, float *gold)
{
	int i;
	int j;
	unsigned errors = 0;
...
static void init_buffer(token_t *in, float * gold)
{
	...
	{//z_dim + x_dim + x_dim * x_dim * 3 + z_dim * z_dim + z_dim * x_dim
		...
		if(i == 0) //only for first iteration
		{
			...
			for(; j < x_dim; j++)
			{
				in[i * in_words_adj + j] = (token_t) float_to_fixed32(initial[j], 16);
			}

			//P
			for(; j < x_dim + x_dim * x_dim; j++)
			{
				in[i * in_words_adj + j] = (token_t) float_to_fixed32(0.0, 16);
			}

			//F
			for(; j < x_dim + x_dim * x_dim * 2; j++)
			{
				in[i * in_words_adj + j] = (token_t) float_to_fixed32(A[j - (x_dim + x_dim * x_dim)], 16);
			}

			//Q
			for(; j < x_dim + x_dim * x_dim * 3; j++)
			{
				in[i * in_words_adj + j] = (token_t) float_to_fixed32(W[j - (x_dim + x_dim * x_dim * 2)], 16);
			}

			//R
			for(; j < x_dim + x_dim * x_dim * 3 + z_dim * z_dim; j++)
			{
				in[i * in_words_adj + j] = (token_t) float_to_fixed32(Q[j - (x_dim + x_dim * x_dim * 3)], 16);
			}

			//H
			for(; j < x_dim + x_dim * x_dim * 3 + z_dim * z_dim + z_dim * x_dim; j++)
			{
				in[i * in_words_adj + j] = (token_t) float_to_fixed32(H[j - (x_dim + x_dim * x_dim * 3 + z_dim * z_dim)], 16);
		...
		if(i == 0)
			for(; j < base_index + z_dim * chunks; j++)
			{
				...
				in[j] = (token_t) float_to_fixed32(measurements[NEURONS * (i+1) + j - base_index], 16);
			...
			{
				in[in_words_adj + i * in_words_adj_z + j] = (token_t) float_to_fixed32(measurements[NEURONS * (i * chunks + 1) + j], 16);
...
int main(int argc, char **argv)
{
	...
	/* 	x_dim = atoi(x_str); */
	/* 	((struct kalman_fx_vivado_access*) cfg_000[0].esp_desc)->x_dim = x_dim; */
	/* 	const char* z_str = argv[2]; */
	/* 	z_dim = atoi(z_str); */
	/* 	((struct kalman_fx_vivado_access*) cfg_000[0].esp_desc)->z_dim = z_dim; */
	/* 	const char* iter_str = argv[3]; */
	/* 	iter = atoi(iter_str); */
	/* 	((struct kalman_fx_vivado_access*) cfg_000[0].esp_desc)->iter = iter; */
	/* 	const char* chunks_str = argv[4]; */
	/* 	chunks = atoi(chunks_str); */
	/* 	((struct kalman_fx_vivado_access*) cfg_000[0].esp_desc)->chunks = chunks; */
	/* 	const char* inv_num_str = argv[5]; */
	/* 	inv_num = atoi(inv_num_str); */
	/* 	((struct kalman_fx_vivado_access*) cfg_000[0].esp_desc)->inv_num = inv_num; */
	/* 	const char* inv_reset_str = argv[6]; */
	/* 	inv_reset = atoi(inv_reset_str); */
	/* 	((struct kalman_fx_vivado_access*) cfg_000[0].esp_desc)->inv_reset = inv_reset; */
	/* 	printf("running x_dim=%d z_dim=%d iter=%d chunks=%d inv_num=%d inv_reset=%d \n", x_dim, z_dim, iter, chunks, inv_num, inv_reset); */
	/* } */
	...
	float mid_MSE = 0;
	int best_inv_num = 1;
	int best_inv_reset = 0;
	int worst_inv_num = 1;
	int worst_inv_reset = 0;
	int mid_inv_num = 1;
	int mid_inv_reset = 0;

	//Run all configuration options
	for(int inv_num = 1; inv_num < 7; inv_num++)
		for(int inv_reset = 0; inv_reset < 7; inv_reset++)
		{
			((struct kalman_fx_vivado_access*) cfg_000[0].esp_desc)->inv_num = inv_num;
			((struct kalman_fx_vivado_access*) cfg_000[0].esp_desc)->inv_reset = inv_reset;
			printf("running x_dim=%d z_dim=%d iter=%d chunks=%d inv_num=%d inv_reset=%d \n", x_dim, z_dim, iter, chunks, inv_num, inv_reset);
			...
			printf("\n====== %s ======\n\n", cfg_000[0].devname);
			/* <<--print-params-->> */
			printf("  .inv_reset = %d\n", ((struct kalman_fx_vivado_access*) cfg_000[0].esp_desc)->inv_reset);
			printf("  .inv_num = %d\n", ((struct kalman_fx_vivado_access*) cfg_000[0].esp_desc)->inv_num);
			printf("  .chunks = %d\n", ((struct kalman_fx_vivado_access*) cfg_000[0].esp_desc)->chunks);
			printf("  .iter = %d\n", ((struct kalman_fx_vivado_access*) cfg_000[0].esp_desc)->iter);
			printf("  .x_dim = %d\n", ((struct kalman_fx_vivado_access*) cfg_000[0].esp_desc)->x_dim);
			printf("  .z_dim = %d\n", ((struct kalman_fx_vivado_access*) cfg_000[0].esp_desc)->z_dim);
			printf("\n  ** START **\n");
			...
			float MSE = accuracy_validate_buffer(&buf[out_offset], gold);

			if(inv_num == 1 && inv_reset == 0)
			{
				min_MSE = MSE;
				best_inv_num = inv_num;
				best_inv_reset = inv_reset;
			}
			else
			{
				if(MSE < min_MSE)
			...
			{
				max_MSE = MSE;
				worst_inv_num = inv_num;
				worst_inv_reset = inv_reset;
			}

			if(MSE > mid_MSE && MSE < 1.0)
			{
				mid_MSE = MSE;
				mid_inv_num = inv_num;
				mid_inv_reset = inv_reset;
			}
	...
	printf("Best accracy for %s is %.20f\n", cfg_000[0].devname, min_MSE);
	printf("inv_num %d inv_reset %d\n", best_inv_num, best_inv_reset);

	printf("Worst accracy for %s is %.20f\n", cfg_000[0].devname, max_MSE);
	printf("inv_num %d inv_reset %d\n", worst_inv_num, worst_inv_reset);

	printf("Middle accracy for %s is %.20f\n", cfg_000[0].devname, mid_MSE);
	printf("inv_num %d inv_reset %d\n", mid_inv_num, mid_inv_reset);
