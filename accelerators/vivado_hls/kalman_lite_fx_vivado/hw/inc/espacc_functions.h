...
void compute_int2(comp_word_t inter2[X_MAX][X_MAX], comp_word_t Q_kal[X_MAX][X_MAX], unsigned x_dim)
{
...
void compute_sr(comp_word_t S[Z_MAX][Z_MAX], comp_word_t R_kal[Z_MAX][Z_MAX], unsigned z_dim)
{
...
void init_s_inv(comp_word_t S_inv[Z_MAX][Z_MAX], unsigned z_dim)
{
...
void compute_y(comp_word_t Y[Z_MAX][1], comp_word_t Z[CHUNK_MAX][Z_MAX], unsigned z_dim, unsigned curr_chunk)
{
...
void compute_x_pred(comp_word_t X_pred[1][X_MAX], comp_word_t inter6[1][X_MAX], unsigned x_dim)
{
...
void compute_int7(comp_word_t inter7[X_MAX][X_MAX], unsigned x_dim)
{
...
void compute_out(comp_word_t _outbuff[SIZE_OUT_CHUNK_DATA], comp_word_t X_pred[1][X_MAX], comp_word_t P_pred[X_MAX][X_MAX], unsigned x_dim, unsigned curr_chunk)
{
...
void mat_mul(comp_word_t A[X_DIM][Z_DIM], comp_word_t B[Z_DIM][Z_DIM], comp_word_t C[X_DIM][Z_DIM], unsigned x_dim, unsigned z_dim)
{
...
template<unsigned X_DIM, unsigned Z_DIM>
void mat_mul_sub(comp_word_t A[X_DIM][Z_DIM], comp_word_t B[Z_DIM][Z_DIM], comp_word_t C[X_DIM][Z_DIM], unsigned x_dim, unsigned z_dim, comp_word_t sub)
{
...
void iterative_inverse(comp_word_t new_mat[DIM][DIM], comp_word_t out[DIM][DIM], comp_word_t new_out_final[DIM][DIM], unsigned z_dim)
{
	comp_word_t new_out[DIM][DIM];// = {{0}};
	...
	/* hls::matrix_multiply_top<hls::NoTranspose, hls::NoTranspose, */
        ...
        /*                      Z_MAX, Z_MAX, */
        /*                      TRAITS_K, */
        /*                      comp_word_t, comp_word_t> (new_mat, out, new_out); */
	...
	/* hls::matrix_multiply_top<hls::NoTranspose, hls::NoTranspose, */
        ...
        /*                      Z_MAX, Z_MAX, */
        /*                      TRAITS_K, */
        /*                      comp_word_t, comp_word_t> (out, new_out, new_out_final); */
    /* 	printf("final matrix = \n"); */
    /* hls::print_matrix<Z_MAX, Z_MAX, comp_word_t, hls::NoTranspose>((comp_word_t(*)[Z_MAX])new_out_final, "   "); */
}

template<unsigned DIM>
void iterative_inverse_lite(comp_word_t new_mat[DIM][DIM], comp_word_t out[DIM][DIM], unsigned z_dim)
{
	comp_word_t new_out[DIM][DIM];// = {{0}};
	//float new_out_final[DIM][DIM] = {{0}};
	//float I2[DIM][DIM];
	...
	for(int i = 0; i < z_dim; i++)
		...
		for(int j = 0; j < z_dim; j++){
         #pragma HLS loop_tripcount max=164
			...
			/* if(i < z_dim && j < z_dim) */
   			        comp_word_t tmp2 = out[i][j];
   			        comp_word_t tmp = new_out[i][j];
				comp_word_t result;
				if(i == j){
					result = 2 - tmp;
				}
...
template<unsigned DIM, unsigned ITER>
	void many_iterative_inverse(comp_word_t new_mat[DIM][DIM], comp_word_t out[DIM][DIM], unsigned z_dim)
{

	comp_word_t new_out_final[DIM][DIM];
...
template<unsigned DIM>
void iterative_inverse2(comp_word_t new_mat[DIM][DIM], comp_word_t out[DIM][DIM], comp_word_t final_out[DIM][DIM],comp_word_t final_out2[DIM][DIM], unsigned z_dim)
{
/* #pragma HLS dataflow */
...
template<unsigned DIM>
void iterative_inverse3(comp_word_t new_mat[DIM][DIM], comp_word_t out[DIM][DIM], comp_word_t final_out[DIM][DIM],comp_word_t final_out2[DIM][DIM], unsigned z_dim)
{
/* #pragma HLS dataflow */
    iterative_inverse<DIM>(new_mat, out, final_out, z_dim);
    iterative_inverse<DIM>(new_mat, final_out, final_out2, z_dim);
