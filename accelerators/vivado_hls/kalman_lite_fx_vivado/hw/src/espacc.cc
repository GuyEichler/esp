...
void load(comp_word_t Z[CHUNK_MAX][Z_MAX],
          comp_word_t X[1][X_MAX],
          comp_word_t P[X_MAX][X_MAX],
          comp_word_t F[X_MAX][X_MAX],
          comp_word_t Q_kal[X_MAX][X_MAX],
          comp_word_t R_kal[Z_MAX][Z_MAX],
          comp_word_t H[Z_MAX][X_MAX],
...
load_data:
    ...
    unsigned length_S_inv = z_dim * z_dim;
    ...
    unsigned S_inv_index = H_index + length_H;
    unsigned Z_index = S_inv_index + length_S_inv;// + index_z;
...
//             }
            ...
            else if (total_index < S_inv_index){//H
            ...
            else if (total_index < Z_index){//S_inv
                col_index = total_index - S_inv_index - row*z_dim;
                S_inv[row][col_index] = tmp[j];
                if(col_index == z_dim - 1)
                    row++;
                if(row == z_dim)
                    row = 0;
...
void compute(comp_word_t Z[CHUNK_MAX][Z_MAX],
             comp_word_t X[1][X_MAX],
             comp_word_t P[X_MAX][X_MAX],
             comp_word_t F[X_MAX][X_MAX],
             comp_word_t Q_kal[X_MAX][X_MAX],
             ...
             bool &enable)
{
    ...
    comp_word_t inter1[X_MAX][X_MAX];//Define inter1 with the size of inter3 to be
    ...
    static comp_word_t X_pred_ping[1][X_MAX];
    static comp_word_t X_pred_pong[1][X_MAX];
    static comp_word_t P_pred_ping[X_MAX][X_MAX];
    static comp_word_t P_pred_pong[X_MAX][X_MAX];
    static bool pingpong = true;
    static bool inv_pingpong = false;
    ...
    {
        comp_word_t tmp_pred_x;
        comp_word_t tmp_pred_p;
        if(i == 0 && j < x_dim){
            tmp_pred_x = X[0][j];
            if(pingpong){
...
// #endif
    ...
    mul_int11:hls::matrix_multiply_top<hls::NoTranspose, hls::NoTranspose,
                             ...
                             X_MAX, X_MAX,
                             TRAITS_A,
                             comp_word_t, comp_word_t> (F, P_pred_ping, inter1);
    else
    mul_int12:hls::matrix_multiply_top<hls::NoTranspose, hls::NoTranspose,
                             X_MAX, X_MAX, X_MAX, X_MAX,
                             X_MAX, X_MAX,
                             TRAITS_A,
                             comp_word_t, comp_word_t> (F, P_pred_pong, inter1);
    ...
    hls::matrix_multiply_top<hls::NoTranspose, hls::Transpose,
                             ...
                             X_MAX, X_MAX,
                             TRAITS_B,
                             comp_word_t, comp_word_t> (inter1, F, inter2);
    ...
    hls::matrix_multiply_top<hls::NoTranspose, hls::Transpose,
                             ...
                             Z_MAX, Z_MAX,
                             TRAITS_D,
                             comp_word_t, comp_word_t> (inter3, H, S);
    ...
    //     {
    //         //if(inv_reset > 1 && (batch % inv_reset == 0))
    //         if(i == j && i < z_dim)
    //         {
    //             if(curr_batch == 0 && c == 0)// || inv_num == 0)// || inv_counter == inv_reset)
    //             {
    //                 //S_inv[i][j] = 1.0;
    //                 S_inv_final[i][j] = 1.0; //S_inv[i][j];
    ...
    //             // //else if ((curr_batch != 0 || c > 1) && (inv_counter > 1 || inv_reset == 0))
    //             // {//Comment this block to change policy
    //             //     //word_t tmp = S_inv_final[i][j]; //In different policy
    //             //     //change to S_inv
    //             //     word_t tmp2 = S_inv_final2[i][j];
    //             //     if(inv_num % 2 == 0)// && c > 1)
    //             //         S_inv_final[i][j] = tmp2;
    //             //     // else// if (c > 1)
    //             //     //     S_inv[i][j] = tmp;
    //             // }
...
// #endif
    ...
    // if((curr_batch == 0 && c == 0) || inv_num == 0 || inv_counter == inv_reset)
    // {
    //     //S_inv is pre-calculated in SSK - no computation
    ...
    // }else{
        //S_inv can be improved with Newton
    ...
    //In SSK always run the SSK first (S_inv) then improve with approximation
    // if(inv_reset != 1)
...
// #pragma HLS loop_tripcount max=3
        ...
        //     }
        // }
        // else //inv_counter < inv_reset
        // {
        // if(inv_pingpong == false){ //inv_counter == inv_reset){
        iterative_inverse_lite<Z_MAX>(S, S_inv, z_dim);
            // inv_pingpong = true;
            //printf("HERE\n");
        // }
        // else if(inv_pingpong == true){
        //     iterative_inverse<Z_MAX>(S, S_inv_final, S_inv, z_dim);
        //     inv_pingpong = false;
        // }
        // else if(inv_pingpong == false){// if(i % 2 == 0){
        //     iterative_inverse<Z_MAX>(S, S_inv_final2, S_inv_final, z_dim);
        //     inv_pingpong = true;
        // }
        // }
    ...
    hls::matrix_multiply_top<hls::NoTranspose, hls::Transpose,
                             ...
                             Z_MAX, 1,
                             TRAITS_F,
                             comp_word_t, comp_word_t> (inter5, X_pred_ping, Y);
    else
    hls::matrix_multiply_top<hls::NoTranspose, hls::Transpose,
                             Z_MAX, X_MAX, 1, X_MAX,
                             Z_MAX, 1,
                             TRAITS_F,
                             comp_word_t, comp_word_t> (inter5, X_pred_pong, Y);
...
// #endif
    ...
    hls::matrix_multiply_top<hls::NoTranspose, hls::Transpose,
                             ...
                             1, X_MAX,
                             TRAITS_H,
                             comp_word_t, comp_word_t> (X_pred_ping, F, X_pred_pong);
    else
    hls::matrix_multiply_top<hls::NoTranspose, hls::Transpose,
                             1, X_MAX, X_MAX, X_MAX,
                             1, X_MAX,
                             TRAITS_H,
                             comp_word_t, comp_word_t> (X_pred_pong, F, X_pred_ping);
...
// #endif
    ...
    hls::matrix_multiply_top<hls::NoTranspose, hls::NoTranspose,
                             ...
                             X_MAX, X_MAX,
                             TRAITS_A,
                             comp_word_t, comp_word_t> (inter7, inter2, P_pred_pong);
    else
    hls::matrix_multiply_top<hls::NoTranspose, hls::NoTranspose,
                             X_MAX, X_MAX, X_MAX, X_MAX,
                             X_MAX, X_MAX,
                             TRAITS_A,
                             comp_word_t, comp_word_t> (inter7, inter2, P_pred_ping);
	 ...
	 dma_info_t &load_ctrl, dma_info_t &store_ctrl)
{
    ...
    /* <<--local-params-->> */
         const unsigned inv_reset = 0; //conf_info_inv_reset;
	 ...
	 const unsigned z_dim = conf_info_z_dim;
         ...
         static comp_word_t Z[CHUNK_MAX][Z_MAX];
         static comp_word_t X[1][X_MAX];
         static comp_word_t P[X_MAX][X_MAX];
         static comp_word_t F[X_MAX][X_MAX];
         static comp_word_t Q_kal[X_MAX][X_MAX];
         static comp_word_t R_kal[Z_MAX][Z_MAX];
         static comp_word_t H[Z_MAX][X_MAX];

         //Store the SSK inverse
         static comp_word_t S_inv[Z_MAX][Z_MAX];
