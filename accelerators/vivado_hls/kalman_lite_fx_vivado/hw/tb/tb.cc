...
int main(int argc, char **argv) {
    ...
    Matrix<float, NEURONS, NEURONS> Mat_S_inv;
    float SSK_inv[Z_MAX * Z_MAX];
    float S_inv[Z_MAX * Z_MAX];
    ...
    for(int i = 0; i < SSK_TIME_STAMPS; i++)
    {
        ...
        if(i < TIME_STAMPS)
        {
            ...
            for(unsigned j = 0; j < x_dim + x_dim * x_dim; j++)
            {
                if(j < x_dim)
                {
                    ...
                    outbuff_gold[i/DOWNSMPL * out_words_adj/chunks + j] = prediction[STATES * (i+1) + j];
                }
                else
                {
                    // outbuff_gold[i/DOWNSMPL * out_words_adj/chunks + j] =
                    // (word_t) P_flat[j - x_dim];
                    outbuff_gold[i/DOWNSMPL * out_words_adj/chunks + j] = P_flat[j - x_dim];
                }
            }
        }
        ...
        //Save first S_inv for lite version
        if(i == 0)
    ...
    for(int i = 0; i < NEURONS; i++)
        for(int j = 0; j < NEURONS; j++)
            S_inv[i * NEURONS + j] = Mat_S_inv(i, j);


    // std::ofstream myfile ("S_inv.h");
    // if (myfile.is_open())
    // {
    //     myfile << "float S_inv[] = {";
    // }
    ...
    //         if (myfile.is_open())
    //         {
    //             if(i == NEURONS-1 && j == NEURONS - 1)
    //                 myfile << Mat_S_inv(i,j) << "}; ";
    //             else
    //                 myfile << Mat_S_inv(i,j) << ", ";
    //         }
    //     }
    ...
    {//z_dim + x_dim + x_dim * x_dim * 3 + z_dim * z_dim + z_dim * x_dim
        ...
        if(i == 0) //only for first iteration
        {
            ...
            for(; j < x_dim + x_dim * x_dim * 2; j++)
            {
                ...
                // inbuff[i * in_words_adj + j] = (word_t) A[j - (x_dim + x_dim
                // * x_dim)];// + rand_val;
            ...
            for(; j < x_dim + x_dim * x_dim * 3; j++)
            {
                ...
                // inbuff[i * in_words_adj + j] = (word_t) W[j - (x_dim + x_dim
                // * x_dim * 2)];// + rand_val;
            ...
            for(; j < x_dim + x_dim * x_dim * 3 + z_dim * z_dim; j++)
            {
                ...
                // inbuff[i * in_words_adj + j] = (word_t) Q[j - (x_dim + x_dim
                // * x_dim * 3)];// + rand_val;
            ...
            for(; j < x_dim + x_dim * x_dim * 3 + z_dim * z_dim + z_dim * x_dim; j++)
            {
                ...
                // inbuff[i * in_words_adj + j] = (word_t) H[j - (x_dim + x_dim
                // * x_dim * 3 + z_dim * z_dim)];// + rand_val;
            ...
            for(; j < x_dim + x_dim * x_dim * 3 + z_dim * z_dim + z_dim * x_dim + z_dim * z_dim; j++)
            {
                // inbuff[i * in_words_adj + j] = (word_t) S_inv[j - (x_dim +
                // x_dim * x_dim * 3 + z_dim * z_dim + z_dim * x_dim)];// +
                // rand_val;
                inbuff[i * in_words_adj + j] = S_inv[j - (x_dim + x_dim * x_dim * 3 + z_dim * z_dim + z_dim * x_dim)];// + rand_val;
                //printf("Value of H = %f \n", measurements[NEURONS * (i+1) + j]);
            }
        }
        ...
        if (i == 0)
            for(; j < base_index + z_dim * chunks; j++)
            {
                ...
                // inbuff[j] = (word_t) measurements[NEURONS * (i+1) + j2 -
                // base_index];// * SCALE + rand_val;
                inbuff[j] = measurements[NEURONS * (i+1) + j2 - base_index];// * SCALE + rand_val;
            ...
            for(; j < z_dim * chunks; j++)
            {
                ...
                // inbuff[in_words_adj + i * in_words_adj_z + j] = (word_t)
                // measurements[NEURONS * (i*chunks+1) + j2];// * SCALE +
                // rand_val;
                inbuff[in_words_adj + i * in_words_adj_z + j] = measurements[NEURONS * (i*chunks+1) + j2];// * SCALE + rand_val;
