...
#define SLD_KALMAN_SSK 0x249
#define DEV_NAME "sld,kalman_ssk_vivado"
...
int main(int argc, char * argv[])
{
	...
	ndev = probe(&espdevs, VENDOR_SLD, SLD_KALMAN_SSK, DEV_NAME);
	if (ndev == 0) {
		printf("kalman_ssk not found\n");
		return 0;
	}
	...
	for (n = 0; n < ndev; n++) {
...
#else
		{
			...
			/* <<--regs-config-->> */
			iowrite32(dev, KALMAN_SSK_INV_RESET_REG, inv_reset);
			iowrite32(dev, KALMAN_SSK_INV_NUM_REG, inv_num);
			iowrite32(dev, KALMAN_SSK_CHUNKS_REG, chunks);
			iowrite32(dev, KALMAN_SSK_ITER_REG, iter);
			iowrite32(dev, KALMAN_SSK_X_DIM_REG, x_dim);
			iowrite32(dev, KALMAN_SSK_Z_DIM_REG, z_dim);
