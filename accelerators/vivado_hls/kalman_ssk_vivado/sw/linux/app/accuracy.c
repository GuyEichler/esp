...
static float accuracy_validate_buffer(token_t *out, token_t *gold)
{
	int i;
	int j;
	unsigned errors = 0;
...
static void init_buffer(token_t *in, token_t * gold)
{
	...
	{//z_dim + x_dim + x_dim * x_dim * 3 + z_dim * z_dim + z_dim * x_dim
		...
		if(i == 0) //only for first iteration
		{
			...
			{
				in[i * in_words_adj + j] = (token_t) SSK_inv[j - (x_dim + x_dim * x_dim * 3 + z_dim * z_dim + z_dim * x_dim)];// + rand_val;
				//printf("Value of H = %f \n", measurements[NEURONS * (i+1) + j]);
			}
		}
...
static void init_parameters()
{
	if (DMA_WORD_PER_BEAT(sizeof(token_t)) == 0) {
		in_words_adj = x_dim + x_dim * x_dim * 3 + 2 * z_dim * z_dim + z_dim * x_dim;
		in_words_adj_z = z_dim * chunks;
		out_words_adj = (x_dim + x_dim * x_dim) * chunks;
	} else {
		in_words_adj = round_up(x_dim + x_dim * x_dim * 3 + 2 * z_dim * z_dim + z_dim * x_dim, DMA_WORD_PER_BEAT(sizeof(token_t)));
...
int main(int argc, char **argv)
{
	...
	/* 	x_dim = atoi(x_str); */
	/* 	((struct kalman_ssk_vivado_access*) cfg_000[0].esp_desc)->x_dim = x_dim; */
	/* 	const char* z_str = argv[2]; */
	/* 	z_dim = atoi(z_str); */
	/* 	((struct kalman_ssk_vivado_access*) cfg_000[0].esp_desc)->z_dim = z_dim; */
	/* 	const char* iter_str = argv[3]; */
	/* 	iter = atoi(iter_str); */
	/* 	((struct kalman_ssk_vivado_access*) cfg_000[0].esp_desc)->iter = iter; */
	/* 	const char* chunks_str = argv[4]; */
	/* 	chunks = atoi(chunks_str); */
	/* 	((struct kalman_ssk_vivado_access*) cfg_000[0].esp_desc)->chunks = chunks; */
	/* 	const char* inv_num_str = argv[5]; */
	/* 	inv_num = atoi(inv_num_str); */
	/* 	((struct kalman_ssk_vivado_access*) cfg_000[0].esp_desc)->inv_num = inv_num; */
	/* 	const char* inv_reset_str = argv[6]; */
	/* 	inv_reset = atoi(inv_reset_str); */
	/* 	((struct kalman_ssk_vivado_access*) cfg_000[0].esp_desc)->inv_reset = inv_reset; */
	/* 	printf("running x_dim=%d z_dim=%d iter=%d chunks=%d inv_num=%d inv_reset=%d \n", x_dim, z_dim, iter, chunks, inv_num, inv_reset); */
	/* } */
	...
	for(int inv_num = 0; inv_num < 7; inv_num++)
		for(int inv_reset = 0; inv_reset < 7; inv_reset++)
	{
		...
		printf("\n====== %s ======\n\n", cfg_000[0].devname);
		/* <<--print-params-->> */
		printf("  .inv_reset = %d\n", ((struct kalman_ssk_vivado_access*) cfg_000[0].esp_desc)->inv_reset);
		printf("  .inv_num = %d\n", ((struct kalman_ssk_vivado_access*) cfg_000[0].esp_desc)->inv_num);
		printf("  .chunks = %d\n", ((struct kalman_ssk_vivado_access*) cfg_000[0].esp_desc)->chunks);
		printf("  .iter = %d\n", ((struct kalman_ssk_vivado_access*) cfg_000[0].esp_desc)->iter);
		printf("  .x_dim = %d\n", ((struct kalman_ssk_vivado_access*) cfg_000[0].esp_desc)->x_dim);
		printf("  .z_dim = %d\n", ((struct kalman_ssk_vivado_access*) cfg_000[0].esp_desc)->z_dim);
		printf("\n  ** START **\n");
