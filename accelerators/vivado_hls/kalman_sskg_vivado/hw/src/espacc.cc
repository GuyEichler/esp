...
load_data:
    ...
    unsigned total_length =
        round_up(x_dim + x_dim * x_dim * 3 + z_dim * x_dim + z_dim * x_dim, VALUES_PER_WORD);
    ...
    unsigned H_index = Q_index + length_Q; //R_index + length_R;
    unsigned K_index = H_index + length_H;
    unsigned Z_index = K_index + length_K;// + index_z;
...
//             }
            ...
            else if (total_index < Z_index){//K
                col_index = total_index - K_index - row*z_dim;
                K[row][col_index] = tmp[j];
                if(col_index == z_dim - 1)
                    row++;
                if(row == x_dim)
                    row = 0;
             ...
             bool &enable)
{
    ...
    //         if(i == j && i < z_dim)
    //         {
    //             if((curr_batch == 0 && c == 0) || inv_counter == inv_reset)// || inv_num == 0)// || inv_counter == inv_reset)
    //             {
    //                 //S_inv[i][j] = 1.0;
