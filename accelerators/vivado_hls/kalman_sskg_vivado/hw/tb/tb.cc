...
int main(int argc, char **argv) {
    ...
    in_words_adj = round_up(x_dim + x_dim * x_dim * 3 + z_dim * x_dim + z_dim * x_dim, VALUES_PER_WORD);
    ...
    //prepare SSK gain
    Matrix<float, STATES, NEURONS> Mat_SSK_gain;
    float K[X_MAX * Z_MAX];
    ...
    for(int i = 0; i < STATES; i++)
        for(int j = 0; j < NEURONS; j++)
            K[i * NEURONS + j] = Mat_SSK_gain(i, j);
    ...
    {//z_dim + x_dim + x_dim * x_dim * 3 + z_dim * z_dim + z_dim * x_dim
        ...
        if(i == 0) //only for first iteration
        {
            ...
            for(; j < x_dim + x_dim * x_dim * 3 + z_dim * x_dim; j++)
            {
                ...
                inbuff[i * in_words_adj + j] = (word_t) H[j - (x_dim + x_dim * x_dim * 3)];// + rand_val;
                //printf("Value of H = %f \n", measurements[NEURONS * (i+1) + j]);
            }

            //SSK gain
            for(; j < x_dim + x_dim * x_dim * 3 + z_dim * x_dim + x_dim * z_dim; j++)
            {
                inbuff[i * in_words_adj + j] = (word_t) K[j - (x_dim + x_dim * x_dim * 3 + z_dim * x_dim)];// + rand_val;
                //printf("Value of H = %f \n", measurements[NEURONS * (i+1) + j]);
            }
        }

        unsigned base_index = (x_dim + x_dim * x_dim * 3 + z_dim * x_dim + z_dim * x_dim);
        //Z
    ...
    for(unsigned i = 0; i < iter; i++)
        for(unsigned j = 0; j < (x_dim + x_dim * x_dim)*chunks; j++)
        {
            ...
            if(j%(x_dim + x_dim*x_dim) < x_dim){
                ...
                //     }
                // }
                word_t norm_diff_out = std::abs(diff/acc_val) > std::abs(diff/gold_val) ? std::abs(diff/acc_val) : std::abs(diff/gold_val);
                if(norm_diff_out > max_diff_prediction)
                {
                    max_diff_prediction = norm_diff_out;
	    ...
	    if (outbuff[i * out_words_adj + j] != outbuff_gold[i * out_words_adj + j]){
                if(std::abs(diff/gold_val) > thresh || diff/acc_val > thresh || diff/gold_val < (-1)*thresh || diff/acc_val < (-1) * thresh){
                    //printf("Accelerator value: %f Golden value: %f index: %d\n", acc_val, gold_val, i * out_words_adj + j);
                    if(j < x_dim)
    ...
    std::cout << "Output max_diff prediction: " << max_diff_prediction << std::endl;
