...
#define SLD_KALMAN_SSKG 0x349
#define DEV_NAME "sld,kalman_sskg_vivado"
...
static void init_buf (token_t *in, token_t * gold)
{
	...
	{//z_dim + x_dim + x_dim * x_dim * 3 + z_dim * z_dim + z_dim * x_dim
		...
		if(i == 0) //only for first iteration
		{
			...
			{
				in[j] = (token_t) H[j - (x_dim + x_dim * x_dim * 3)];
				//printf("Value of H = %f \n", measurements[NEURONS * (i+1) + j]);
			}

			//SSK gain
			for(; j < x_dim + x_dim * x_dim * 3 + z_dim * x_dim + z_dim * x_dim; j++)
			{
...
int main(int argc, char * argv[])
{
	...
	if (DMA_WORD_PER_BEAT(sizeof(token_t)) == 0) {
		in_words_adj = x_dim + x_dim * x_dim * 3 + z_dim * x_dim + z_dim * x_dim;
	...
	ndev = probe(&espdevs, VENDOR_SLD, SLD_KALMAN_SSKG, DEV_NAME);
	if (ndev == 0) {
		printf("kalman_sskg not found\n");
		return 0;
	}
	...
	for (n = 0; n < ndev; n++) {
...
#else
		{
			...
			/* <<--regs-config-->> */
			iowrite32(dev, KALMAN_SSKG_INV_RESET_REG, inv_reset);
			iowrite32(dev, KALMAN_SSKG_INV_NUM_REG, inv_num);
			iowrite32(dev, KALMAN_SSKG_CHUNKS_REG, chunks);
			iowrite32(dev, KALMAN_SSKG_ITER_REG, iter);
			iowrite32(dev, KALMAN_SSKG_X_DIM_REG, x_dim);
			iowrite32(dev, KALMAN_SSKG_Z_DIM_REG, z_dim);
