...
load_data:
    ...
    unsigned total_length =
        round_up(x_dim + x_dim * x_dim * 3 + 2 * z_dim * z_dim + z_dim * x_dim, VALUES_PER_WORD);
    ...
    unsigned C_index = H_index + length_H;
    unsigned Z_index = C_index + length_C;// + index_z;
...
//             }
            ...
            else if (total_index < Z_index){//C
                col_index = total_index - C_index - row*z_dim;
                C[row][col_index] = tmp[j];
                if(col_index == z_dim - 1)
                    row++;
                if(row == z_dim)
                    row = 0;
...
store_data:
    ...
    const unsigned store_offset =
        round_up(x_dim + x_dim * x_dim * 3 + z_dim * z_dim + z_dim * x_dim + z_dim * z_dim , VALUES_PER_WORD) +
             ...
             bool &enable)
{
    ...
    word_t inter3[X_MAX][Z_MAX];//Reuse inter 1
    //word_t S[Z_MAX][Z_MAX];

    //Sum the mat terms
    //word_t E_term[Z_MAX];//[X_MAX];
    word_t final_term[Z_MAX][Z_MAX];
    //word_t Sum[Z_MAX][Z_MAX];

    //Term calculation
    //static word_t C[Z_MAX][Z_MAX];
    static word_t C_H[Z_MAX][X_MAX];
    static word_t H_C[X_MAX][Z_MAX];
    ...
    word_t inter8[X_MAX][X_MAX];// Reuse inter1

    word_t inter9[X_MAX][Z_MAX];
    word_t inter10[X_MAX][X_MAX];
    ...
    printf("c = %d\n", c);
    printf("pingpong = %d\n", pingpong);
    printf("inv_pingpong = %d\n", inv_pingpong);
#endif

    //Initialize sum matrix
    LOOP_INIT_FIN1:for(unsigned i = 0; i < z_dim; i++)
    LOOP_INIT_FIN2:for(unsigned j = 0; j < z_dim; j++)
    {
        //initialize the final term as matrix C and remove the sum from it
        word_t tmp = C[i][j];
        final_term[i][j] = tmp; //0.0;
        // if(j < X_MAX)
        //     E_term[i][j] = 0.0;
    }

    // for(unsigned i = 0; i < Z_MAX; i++)
    // {
    //     //initialize the final term as matrix C and remove the sum from it
    //     E_term[i] = 0.0;
    // }
...
// #endif
    ...
    //Compute inter2 = inter2 + Q --- P_pred in inter2
    ...
    //calculate C*H and H.T*C
    // if(c == 0 && curr_batch == 0)
    // {
        //Compute C x H
        hls::matrix_multiply_top<hls::NoTranspose, hls::NoTranspose,
                                 Z_MAX, Z_MAX, Z_MAX, X_MAX,
                                 Z_MAX, X_MAX,
                                 TRAITS_L,
                                 word_t, word_t> (C, H, C_H);
        ...
        hls::matrix_multiply_top<hls::Transpose, hls::NoTranspose,
                                 ...
                                 X_MAX, Z_MAX,
                                 TRAITS_M,
                                 word_t, word_t> (H, C, H_C);
    //}

    LOOP_TERM1:for(unsigned u = 0; u < x_dim; u++)
    {
    LOOP_TERM2:for(unsigned l = 0; l < x_dim; l++)
        {
            word_t scalar = inter2[u][l];
            if(u == l) //Minus identity
                scalar = scalar - 1.0;

            //printf(" scalar (%d,%d) = %f\n", u, l, scalar);

            // //Compute the left matrix term
            // for(unsigned i = 0; i < Z_MAX; i++)
            //     //for(unsigned j = 0; j < X_MAX; j++)
            // {
            //     if(i < z_dim)
            //         //C_H * E
            //         //if(j == l)
            ...
            //Compute the rest matrix term
        LOOP_TERM3:for(unsigned i = 0; i < z_dim; i++)
        //for(unsigned j = 0; j < X_MAX; j++)
            {
                //E_term * H_C
                //if(j == l)
                word_t tmp1 = C_H[i][u];
            LOOP_TERM4:for(unsigned k = 0; k < z_dim; k++)
                {
                #pragma HLS array_partition \
                    variable=H_C cyclic factor=8 dim=2
                    /* #pragma HLS array_partition \ */
                    /* 	variable=A cyclic factor=4 dim=2 */
                #pragma HLS array_partition \
                    variable=final_term cyclic factor=8 dim=2
                    //#pragma HLS dependence variable=H_C inter false
                    //#pragma HLS dependence variable=final_term inter false
                #pragma HLS PIPELINE II=1
                #pragma HLS unroll factor=8
                    //word_t tmp = E_term[i][l];
                    //word_t tmp = E_term[i];
                    // word_t tmp1 = C_H[i][u];
                    word_t tmp2 = H_C[l][k];
                    //word_t scalar = inter2[u][l];
                    word_t previous = final_term[i][k];
                    // if(u == l) //Minus identity
                    //     scalar = scalar - 1.0;
                    //Sum the terms over all (u,l) and remove from matrix C
                    //(pre stored in final_term)
                    word_t tmp3 = tmp1 * tmp2;
                    word_t tmp4 = tmp3 * scalar;
                    final_term[i][k] = previous - tmp4;
                }
            }
        }
    }
...
#ifndef __SYNTHESIS__
        // printf("E_term = \n");
        // hls::print_matrix<Z_MAX, 1, word_t, hls::NoTranspose>((word_t(*)[1])E_term, "   ");
#endif

#ifndef __SYNTHESIS__
        // printf("final_term = \n");
        // hls::print_matrix<Z_MAX, Z_MAX, word_t, hls::NoTranspose>((word_t(*)[Z_MAX])final_term, "   ");
#endif

    //Now we have the final matrix taylor term and we need to compute K

    //Compute K_approx = inter2 * H.T * final_term
    hls::matrix_multiply_top<hls::NoTranspose, hls::Transpose,
                             X_MAX, X_MAX, Z_MAX, X_MAX,
                             X_MAX, Z_MAX,
                             TRAITS_J,
                             word_t, word_t> (inter2, H, inter3);

    hls::matrix_multiply_top<hls::NoTranspose, hls::NoTranspose,
                             X_MAX, Z_MAX, Z_MAX, Z_MAX,
                             X_MAX, Z_MAX,
                             TRAITS_E,
                             word_t, word_t> (inter3, final_term, K);
...
// #endif
    ...
    //Compute P_pred
    //Compute inter8 = inter7 * P_pred * inter7.T
    hls::matrix_multiply_top<hls::NoTranspose, hls::NoTranspose,
                             X_MAX, X_MAX, X_MAX, X_MAX,
                             X_MAX, X_MAX,
                             TRAITS_A,
                             word_t, word_t> (inter7, inter2, inter8);
    ...
    hls::matrix_multiply_top<hls::NoTranspose, hls::Transpose,
                             ...
                             X_MAX, X_MAX,
                             TRAITS_B,
                             word_t, word_t> (inter8, inter7, P_pred_pong);
    else
    hls::matrix_multiply_top<hls::NoTranspose, hls::Transpose,
                             X_MAX, X_MAX, X_MAX, X_MAX,
                             X_MAX, X_MAX,
                             TRAITS_B,
                             word_t, word_t> (inter8, inter7, P_pred_ping);

    //Compute inter10 = K * R * K.T
    hls::matrix_multiply_top<hls::NoTranspose, hls::NoTranspose,
                             X_MAX, Z_MAX, Z_MAX, Z_MAX,
                             X_MAX, Z_MAX,
                             TRAITS_E,
                             word_t, word_t> (K, R_kal, inter9);

    hls::matrix_multiply_top<hls::NoTranspose, hls::Transpose,
                             X_MAX, Z_MAX, X_MAX, Z_MAX,
                             X_MAX, X_MAX,
                             TRAITS_N,
                             word_t, word_t> (inter9, K, inter10);

    //Compute P_pred = inter8 + inter10
    LOOP_P_1:for(int i = 0; i < X_MAX; i++)
    LOOP_P_2:for(int j = 0; j < X_MAX; j++)
        {
            word_t tmp1 = inter10[i][j];
            if(pingpong)
            {
                word_t tmp2 = P_pred_pong[i][j];
                P_pred_pong[i][j] = tmp1 + tmp2;
            }
            else
            {
                word_t tmp2 = P_pred_ping[i][j];
                P_pred_ping[i][j] = tmp1 + tmp2;
            }
        }
...
// #endif
    ...
    // printf("P_pred = \n");
    // if(pingpong)
    //     hls::print_matrix<X_MAX, X_MAX, word_t, hls::NoTranspose>((word_t(*)[X_MAX])P_pred_pong, "   ");
    // else
    //     hls::print_matrix<X_MAX, X_MAX, word_t, hls::NoTranspose>((word_t(*)[X_MAX])P_pred_ping, "   ");
#endif
	 ...
	 dma_info_t &load_ctrl, dma_info_t &store_ctrl)
{
    ...
    /* <<--local-params-->> */
	 ...
	 const unsigned z_dim = conf_info_z_dim;
         ...
         //Store matrix C for taylor approximation
         static word_t C[Z_MAX][Z_MAX];
