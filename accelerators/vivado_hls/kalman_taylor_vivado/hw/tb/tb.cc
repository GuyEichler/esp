...
#define SSK_TIME_STAMPS TIME_STAMPS
...
int main(int argc, char **argv) {
    ...
    //prepare C
    Matrix<float, NEURONS, NEURONS> Mat_C;
    float C[Z_MAX * Z_MAX];

    Mat_C = (Mat_H * Mat_H.transpose() + Mat_R).inverse();
    //std::cout << "Matrix C = " << Mat_C << std::endl;

    for(int i = 0; i < NEURONS; i++)
        for(int j = 0; j < NEURONS; j++)
            C[i * NEURONS + j] = Mat_C(i, j);
    ...
    {//z_dim + x_dim + x_dim * x_dim * 3 + z_dim * z_dim + z_dim * x_dim
        ...
        if(i == 0) //only for first iteration
        {
            ...
            {
                inbuff[i * in_words_adj + j] = (word_t) C[j - (x_dim + x_dim * x_dim * 3 + z_dim * z_dim + z_dim * x_dim)];// + rand_val;
                //printf("Value of H = %f \n", measurements[NEURONS * (i+1) + j]);
            }
        }
