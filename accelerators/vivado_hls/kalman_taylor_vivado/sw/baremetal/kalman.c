...
#define SLD_KALMAN_TAYLOR 0x350
#define DEV_NAME "sld,kalman_taylor_vivado"
...
static int validate_buf(token_t *out, token_t *gold)
{
	...
	for (i = 0; i < iter; i++)
		for (j = 0; j < (x_dim + x_dim * x_dim)*chunks; j++)
		{
			...
			{
				if(diff / gold_val > 0.3 || diff / acc_val > 0.3 || diff / gold_val < -0.3 || diff / acc_val < -0.3){
					if(j < x_dim)
...
int main(int argc, char * argv[])
{
	...
	ndev = probe(&espdevs, VENDOR_SLD, SLD_KALMAN_TAYLOR, DEV_NAME);
	if (ndev == 0) {
		printf("kalman_taylor not found\n");
		return 0;
	}
	...
	for (n = 0; n < ndev; n++) {
...
#else
		{
			...
			/* <<--regs-config-->> */
			iowrite32(dev, KALMAN_TAYLOR_INV_RESET_REG, inv_reset);
			iowrite32(dev, KALMAN_TAYLOR_INV_NUM_REG, inv_num);
			iowrite32(dev, KALMAN_TAYLOR_CHUNKS_REG, chunks);
			iowrite32(dev, KALMAN_TAYLOR_ITER_REG, iter);
			iowrite32(dev, KALMAN_TAYLOR_X_DIM_REG, x_dim);
			iowrite32(dev, KALMAN_TAYLOR_Z_DIM_REG, z_dim);
